...
export function OrchardAnalysisClient({ dataError, records }: { dataError: string | null; records: readonly AnalysisDataRecord[] }) {
  ...
  const valuesTemplate = visibleColumns.map((column) => `${columns[column].width}px`).join(" ");
  ...
  return <main className="orchard-page">
    ...
    <section className="orchard-results" aria-label="園地分析の時系列一覧">
      ...
      {dataError ? <p className="orchard-empty">{dataError}</p> : recordCount === 0 ? <p className="orchard-empty">条件に一致する調査データはありません。</p> : <div className="orchard-table">
        <div className="orchard-column-headings"><div className="orchard-date">日付</div><div className="orchard-scroll-area"><div className="orchard-values" style={{ gridTemplateColumns: valuesTemplate }}>{visibleColumns.map((column) => <span key={column}>{columns[column].label}</span>)}</div></div></div>
        {timeline.map((entry) => entry.type === "year" ? <div className="orchard-year-divider" key={`year-${entry.year}`}>{entry.year}年</div> : <div className="orchard-row" key={entry.row.registrationId}><div className="orchard-date">{displayDate(entry.row.measuredAt)}</div><div className="orchard-scroll-area"><div className="orchard-values" style={{ gridTemplateColumns: valuesTemplate }}>{visibleColumns.map((column) => <span key={column}>{columns[column].value(entry.row)}</span>)}</div></div></div>)}
      </div>}
    </section>
  </main>;
}
