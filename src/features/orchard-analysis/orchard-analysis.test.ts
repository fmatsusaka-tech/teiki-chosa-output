...
describe("buildOrchardAnalysis", () => {
  ...
  it("処理区は指定時のみ絞り込み、未指定時はすべてを返す", () => {
    ...
    expect(buildOrchardAnalysis(records, { orchard: "吉川", variety: "ゆら早生" }).filter((entry) => entry.type === "record")).toHaveLength(2);
    expect(buildOrchardAnalysis(records, { orchard: "吉川", variety: "ゆら早生", treatment: null })).toMatchObject([
