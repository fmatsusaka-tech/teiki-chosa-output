...
const aliasMap = new Map(definitions.flatMap((definition) => definition.aliases.map((alias) => [normalizeVarietyText(alias), definition] as const)));

export const getVarietyCategory = (rawVariety: string | null | undefined): string | null => {
  if (!rawVariety || !normalizeVarietyText(rawVariety)) return null;
